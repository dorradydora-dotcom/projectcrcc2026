          ...
          Row(
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  color:
                      (isOut ? Colors.redAccent : Colors.blue).withValues(alpha: 0.1),
          ...
          ElevatedButton(
            onPressed: () async {
              if (stationController.text.isNotEmpty) {
                await _supabase.from(AppConstants.tableCapacitors).insert({
    ...
    await Get.dialog(
      AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              await _supabase.from(AppConstants.tableCapacitors).update({
                'station_name': stationController.text,
                'voltage_level': voltageController.text,
                'capacity_mvar': double.tryParse(capacityController.text),
                'status': statusController.text,
