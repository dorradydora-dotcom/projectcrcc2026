...
class EventsController extends GetxController {
  ...
  Future<void> addEvent() async {
    ...
    try {
      await _supabase.from(AppConstants.tableEvents).insert({
...
الموقع: ${locationController.text.isEmpty ? 'غير محدد' : locationController.text}
      ...
      await NotificationService.sendNotification(
          AppConstants.tableUserCrcc, 'حدث جديد', eventBody,
          route: 'الاحداث');
      await NotificationService.sendNotification(
          AppConstants.tableUserTop, 'حدث جديد', eventBody,
          route: 'الاحداث');
  ...
  Future<void> updateEvent(Event event) async {
    ...
    try {
      await _supabase.from(AppConstants.tableEvents).update({
        'title': titleController.text,
        'description': descriptionController.text,
        'date': eventDate.toIso8601String(),
        'location': locationController.text,
...
الموقع: ${locationController.text.isEmpty ? 'غير محدد' : locationController.text}
      ...
      await NotificationService.sendNotification(
          AppConstants.tableUserCrcc, 'تعديل حدث', eventBody,
          route: 'الاحداث');
      await NotificationService.sendNotification(
          AppConstants.tableUserTop, 'تعديل حدث', eventBody,
          route: 'الاحداث');
  ...
  Future<void> deleteEvent(
    ...
    try {
      await _supabase.from(AppConstants.tableEvents).delete().eq('id', eventId);
      ...
      await NotificationService.sendNotification(
          AppConstants.tableUserCrcc, 'حذف حدث', deleteBody,
          route: 'الاحداث');
      await NotificationService.sendNotification(
          AppConstants.tableUserTop, 'حذف حدث', deleteBody,
          route: 'الاحداث');
