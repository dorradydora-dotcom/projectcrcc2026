                  ...
                  children: [
                    FadeInRight(
                      child: Container(
                        ...
                        child: Row(
                          children: [
                            Icon(Icons.inventory_2, color: C.gold, size: 16.sp),
                            SizedBox(width: 8.w),
          ...
          Row(
            children: [
              Container(
                ...
                decoration: BoxDecoration(
                  color:
                      (isReserved ? Colors.redAccent : C.blue).withOpacity(0.1),
                  shape: BoxShape.circle,
                ),
                child: Icon(
                  isReserved ? Icons.lock : Icons.check_circle,
                  color: isReserved ? Colors.redAccent : C.blue,
                  size: 16.sp,
                ),
              ),
              SizedBox(width: 8.w),
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              if (stationController.text.isNotEmpty) {
                await _supabase.from('spare_cells').insert({
      ...
      AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            style: ElevatedButton.styleFrom(backgroundColor: C.gold),
            onPressed: () async {
              await _supabase.from('spare_cells').update({
                'station_name': stationController.text,
                'voltage_level': voltageController.text,
                'cell_number': int.tryParse(cellController.text) ?? 0,
                'status': statusController.text,
      ...
      AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () async {
              await _supabase.from('spare_cells').delete().eq('id', item['id']);
              Get.back();
              setState(() {});
            },
